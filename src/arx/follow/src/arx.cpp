...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_joint = nh.subscribe<arx_msgs::JointControl>(
      "joint_control", 10, &jointControlCallback);

  ros::Publisher pub_current =
      nh.advertise<arx_msgs::JointInformation>("joint_information", 10);
  ros::Publisher pub_pos =
      nh.advertise<arx_msgs::PosCmd>("/follow1_pos_back", 10);

  ros::Rate loop_rate(200);

  while (ros::ok()) {
    std::vector<double> joint_pos_vector = r5_interface_ptr->getJointPositons();
    std::vector<double> joint_vel_vector =
        r5_interface_ptr->getJointVelocities();
    // 发送关节数据
